...
那神秘的包含规则是什么呢？不幸的是，我又没找到这些内容的具体描述。唯一提到这些的地方是在 [ziglang/src/Package/Fetch.zig](https://github.com/ziglang/zig/blob/1a6485d111d270014f57c46c53597a516a24dc47/src/Package/Fetch.zig#L1-L28) 文件的开头，但只能了解到被排除的文件将被删除，而哈希值是在剩余文件的基础上计算出来的结果。
...
现在我们知道了 `build.zig.zon` 的包含规则，也知道 `SHA256` 算法是包含在内。但我们仍然不知道实际的哈希结果是如何得到的。例如，它可能是通过将所有包含的文件内容输入哈希器来计算的。所以让我们再仔细看看，也许我们可以找到答案。

回到 `runResource` 函数，我们看到它调用了 [`computeHash`](https://github.com/ziglang/zig/blob/1a6485d111d270014f57c46c53597a516a24dc47/src/Package/Fetch.zig#L1324) 函数，这看起来应该是我们感兴趣的主要内容（它顶部的注释已经无人维护，而里面又进行了[文件删除](https://github.com/ziglang/zig/blob/1a6485d111d270014f57c46c53597a516a24dc47/src/Package/Fetch.zig#L1383-L1385)）。

在其中我们偶然发现了这段[代码](https://github.com/ziglang/zig/blob/1a6485d111d270014f57c46c53597a516a24dc47/src/Package/Fetch.zig#L1404-L1416)：
...
首先，我们有一些设置，其中创建并初始化了一个新的哈希器实例，使用文件的标准化路径：
...
打开文件以便稍后读取其内容，结果和预期差别不大。但紧接着我们放入了两个 null 字节。从阅读相关问题来看，这似乎与一种主要是历史原因而保留下来的做法有关，目的是不使正在使用的哈希失效。无论如何，之后我们简单地循环读取文件数据的块，并将它们作为数据来计算哈希值。
...
我们标准化路径分隔符，并将符号链接的目标路径输入 `hasher`。在 `hashFileFallible` 的末尾，我们在传递的 `HashedFile` 对象中存储计算出的哈希：`hasher.final(&hashed_file.hash);`。

## 组合哈希
尽管有了单个文件的哈希值，但我们仍不知道如何得到最终的哈希。幸运的是，曙光就在眼前。

下一步是确保我们有可复现的结果。 `HashedFile` 对象被存储在一个数组中，但例如文件系统遍历算法可能会改变，所以我们需要对那个数组进行排序。

```zig
std.mem.sortUnstable(*HashedFile, all_files.items, {}, HashedFile.lessThan);
```

最后，我们到达了将所有这些哈希[组合成一个的部分](https://github.com/ziglang/zig/blob/1a6485d111d270014f57c46c53597a516a24dc47/src/Package/Fetch.zig#L1452-L1464)：
...
现在我们有了一个 `SHA256` 摘要，我们可以最终返回到 `main.zig`，在那里我们调用 [`Package.Manifest.hexDigest(fetch.actual_hash)`](https://github.com/ziglang/zig/blob/9d64332a5959b4955fe1a1eac793b48932b4a8a8/src/Package/Manifest.zig#L174)。在那里，我们将多哈希头写入缓冲区，之后是我们的组合摘要。
